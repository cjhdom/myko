...
//         minLevel: $rootScope.static.currentLevel, // 클러스터 할 최소 지도 레벨
    ...
    async doClickCluster (cluster, isRefresh) {
        ...
        try {
            const fetchResult = await fetch('http://kosiwon.com/api/kosiwons/listByIdListWithPaging', {
                method: 'POST',
                headers: fetchHeader,
                body: JSON.stringify({
                    projectOption:  {
    ...
    async doClickMarker (marker) {
        ...
        try {
            const fetchResult = await fetch('http://kosiwon.com/api/kosiwons/listByIdList', {
                method: 'POST',
                headers: fetchHeader,
                body: JSON.stringify({
                    projectOption:  {
